...
class RGBCamera(Driver):
    ...
    defaults["subtract_background"] = True
    defaults["show_background_pipeline"] = False
    defaults["background_threshold"] = 25
    defaults["background_capture_on_init"] = True
    defaults["camera_warmup_delay"] = 0.2
        ...
        **kwargs : dict
        ...
        if plotting:
            try:
                ...
                center_x, center_y = ds.attrs["located_center"]
                ...
                if subtract_background and background_processed is not None:
                    axs[1].imshow(
                        np.where(background_processed["mask"][..., None], 1.0, np.nan),
                        alpha=0.35,
                        cmap="magma",
                    )
                axs[1].add_patch(
                    Circle((center_x, center_y), radius, edgecolor="red", facecolor="none", linewidth=2)
                )
                axs[1].axis("off")

                title = "Detected circle"
                if subtract_background:
                    title = "Detected circle + background subtraction"
                axs[1].set_title(
